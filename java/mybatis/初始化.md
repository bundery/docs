...
> 任何框架的初始化，无非是加载自己运行时所需要的配置信息。Mybatis 初始化时会把 XML 文件的所有配置信息都放在`org.apache.ibatis.session.Configuration`容器对象中，Configuration 对象的属性结构与 XML 配置文件的组织结构几乎完全一样，Mybatis 初始化的 过程，就是创建 Configuration 对象（包含了mybatis-config.xml文件的所有配置信息）的过程。

## 涉及对象

- sqlSession：封装了一次对数据库交互的会话访问，完成数据库增删改查功能及事务控制；
- Executor：MyBatis执行器，是 MyBatis 调度的核心，负责 SQL 语句的生成和查询缓存的维护；
...
- 调用 SqlSessionFactoryBuilder 的 build(inputStream) 方法，根据输入流创建 XMLConfigBuilder 对象；
- 调用 XMLConfigBuilder 对象的 parse() 方法，创建 Configuration 容器对象，并把配置文件里的所有配置信息设置到 Configuration 对象的对应属性，其中包括所有 mapper 配置文件配置的信息（每一个  "Select|Insert|delete|update" 语句都会生成一个 MappedStatement 对象，并把该对象加入到 configuration 的一个 map 属性中）;
- SqlSessionFactoryBuilder 创建 DefaultSessionFactory 对象。

**以下是 Mapper 部分的初始化过程（伪代码）**:
