...
> 框架的初始化，就是加载本身运行时所需要的配置信息。Mybatis 初始化时会把 XML 文件的所有配置信息都放在`org.apache.ibatis.session.Configuration`容器对象中，Configuration 对象的属性结构与 XML 配置文件的组织结构几乎完全一样，Mybatis 初始化的 过程，就是创建 Configuration 对象（包含了mybatis-config.xml文件的所有配置信息）的过程。
